...
const Gallery: React.FC = () => {
  ...
  const categories = ["All", "Medical", "Interior", "Patient Care"];
      ...
      <section className="container mx-auto px-6 mb-24">
        ...
        >
          ...
          <p className="text-xl text-slate-500 max-w-2xl leading-relaxed font-light">
            Browse through the spaces and technology that define the
            Awarihospital experience.
          </p>
        </motion.div>
      </section >
          ...
          <AnimatePresence mode="popLayout">
            {filteredItems.map((item, idx) => (
              <motion.div
                ...
                className="group relative h-[500px] overflow-hidden rounded-[3rem] shadow-xl bg-white border border-slate-100"
              >
                {/* CLICK LAYER */}
                <button
                ...
                {/* Overlay */}
                <div className="pointer-events-none absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-start justify-end p-12 text-white">
      ...
      <AnimatePresence>
        {selectedImage && (
          ...
          >
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-10 right-10 text-white p-4 hover:bg-white/10 rounded-full transition-colors z-[120]"
            >
            ...
            >
              <div className="relative w-full h-full aspect-video">
                <Image
                  ...
                  alt="Selected Image"
                  fill
                  className="pointer-events-none object-contain rounded-[2rem] shadow-2xl w-full h-full"
                />
              </div>
            </motion.div>
          </motion.div>
